...
> [!IMPORTANT]
> 사용자 정의 필터, DNS 전용 필터, 유저스크립트는 역할과 등록하는 메뉴가 서로 다르며 잘못된 설치는 오작동을 유발할 수 있습니다. 차이점에 대해 정확히 인지하고 있지 않으시면 [설치 가이드](./INSTALL.md)를 읽어주세요.
...
* **사진 블러 및 남은 요청 확인**: 게시글 내 이미지 블러 및 `너무 많은 요청`까지 남은 횟수 확인
* **미리보기 섬네일 및 비추천수 표시**: 게시글 리스트에서 본문의 이미지를 확인하고, 비추천수를 함께 표시
* **이용자 메모**: ~~모바일 웹에서도 지원~~ [공지사항](https://m.dcinside.com/board/know/428) 참고
* **주의 사항**: 게시글을 백그라운드에서 접속하므로 셀룰러 데이터 사용량이 증가할 수 있으며 다크 모드는 기기 자체 상태와 연동됩니다.
...
이용 중 발생한 문제, 개선 제안이나 문의는 [GitHub Issues](https://github.com/hooray804/adguard-gallery-filter/issues) 등을 통해 제보해 주시기 바랍니다.

...
* **주의 사항**: **디시인사이드 서버의 Rate Limit 수치가 변경된 것으로 추정되며, 정상적인 스크립트 이용이 불가능해짐에 따라 더 이상 사용을 권장하지 않습니다.** 본 스크립트는 기능 구현을 위해 게시글을 백그라운드에서 접속하므로 셀룰러 데이터 사용량이 증가할 수 있습니다. 다크 모드는 기기 자체 다크 모드 상태와 연동됩니다.

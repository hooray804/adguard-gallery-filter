...
(function() {
        ...
        const modifyNode = (el) => {
            if (!el.dataset.origNick) {
                ...
                for (const n of el.childNodes) {
                    if (n.nodeType === 3 && n.nodeValue.trim()) { nickNode = n; break; }
                }
                if (nickNode) {
                    el.dataset.origNick = nickNode.nodeValue.trim();
                    el.dataset.nickNodeIdx = Array.from(el.childNodes).indexOf(nickNode);
                } else {
                    el.dataset.origNick = el.textContent.trim();
                }
            }
            const orig = el.dataset.origNick;
            if (orig) {
                const memo = memos[orig];
                const targetStr = memo ? `${orig}:${memo}` : orig;
                if (el.dataset.nickNodeIdx !== undefined) {
                    const targetNode = el.childNodes[el.dataset.nickNodeIdx];
                    if (targetNode && targetNode.nodeValue !== targetStr) {
                        targetNode.nodeValue = targetStr;
                    }
                } else {
                    if (el.textContent !== targetStr) el.textContent = targetStr;
                }
            }
        };

        const plate = qs('.side > .member_plate') || qs('.btm_area > .side > .member_plate');
        if (plate) modifyNode(plate);

        qsa("li[id^='comment']").forEach(li => {
            const nickEl = qs('.meta > a[href^="#"]', li);
            if (!nickEl) return;
            const nick = nickEl.textContent.trim();
            const xeContent = qs('.comment-content > .xe_content', li);
            if (!xeContent) return;
            ...
            span.className = 'fmk-memo-span';
            span.textContent = `(메모내용: ${memo})`;
            span.style.color = 'gray';
            const br = doc.createElement('br');

            const findParent = qs('a.findParent', xeContent);
            if (findParent) {
                const br2 = doc.createElement('br');
                findParent.after(br2);
                br2.after(span);
                span.after(br);
            } else {
                xeContent.insertBefore(br, xeContent.firstChild);
                xeContent.insertBefore(span, br);
            }
        });
    };
    ...
    const injectMemoBtn = () => {
        ...
        qsa("li[id^='comment']").forEach(li => {
            ...
            if (voteContainer && !qs('.fmk-memo-btn', voteContainer)) {
                ...
                const nick = a ? a.textContent.trim() : '';

                btn.addEventListener('click', (e) => {
                    e.preventDefault();
                    if (nick) showMemoPopup(nick, e.pageX, e.pageY);
                });
        ...
        if (btmArea) {
            const sideFr = qs('.side.fr', btmArea);
            const plate = qs('.side > .member_plate', btmArea);
    ...
    const showMemoPopup = (nick, x, y) => {
        ...
        if (!popup) {
            ...
            doc.addEventListener('mousedown', (e) => {
                if (!popup.contains(e.target) && !e.target.closest('.fmk-memo-btn')) {
        ...
        const memos = getDict('userMemoDict');
        const currentMemo = memos[nick] || '';

        popup.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;font-weight:bold;">
                <div>${nick.replace(/</g,'&lt;')} 메모</div>
            </div>
            <div style="margin-top:10px;padding-top:10px;border-top:1px dashed ${isDark?'#555':'#ccc'}">
                <div style="display:flex;align-items:center;gap:5px;">
                    <textarea id="fmk-memo-textarea" style="flex-grow:1;width:0;height:34px;line-height:1.5;background:${isDark?'#333':'#f5f5f5'};color:${isDark?'#fff':'#000'};border:1px solid ${isDark?'#555':'#ccc'};border-radius:5px;padding:5px 8px;font-size:14px;resize:none;box-sizing:border-box;margin:0;">${currentMemo}</textarea>
            ...
            <div style="margin-top:15px;padding-top:10px;border-top:1px solid ${isDark?'#444':'#eee'};display:flex;justify-content:flex-end;">
                <div id="fmk-memo-close" style="cursor:pointer;padding:4px;font-size:12px;color:#888;display:flex;align-items:center;">
                    <span style="margin-right:4px;">Close</span>
        ...
        popup.querySelector('#fmk-memo-save').addEventListener('click', () => {
            const val = popup.querySelector('#fmk-memo-textarea').value.trim();
        ...
        popup.querySelector('#fmk-memo-close').addEventListener('click', () => {
            popup.style.display = 'none';
        });
        ...
        let pX = x;
        let pY = y + 15;
        const vRight = window.scrollX + window.innerWidth;
    ...
    const flags = {
        removePowerLink: getVal('removePowerLink'),
        imgEmbed: getVal('imgEmbed'),
        blockUser: getVal('blockUser'),
        blockKeyword: getVal('blockKeyword'),
        useMemo: getVal('useMemo'),
    };

    const mainObserver = new MutationObserver(ms => {
        const doAds = isMobile && flags.removePowerLink && adCheckActive;
        const doEmbed = !isMobile || flags.imgEmbed;
        const doFilter = isMobile && (flags.blockUser || flags.blockKeyword);
        const doMemo = isMobile && flags.useMemo;

        if (!doAds && !doEmbed && !doFilter && !doMemo) return;
    ...
    const renderTags = (sec, key) => {
        ...
        if (key === 'userMemoDict') {
            ...
            Object.keys(dict).forEach(nick => {
                ...
                tag.innerHTML = `${nick.replace(/</g,'&lt;')}: ${dict[nick].replace(/</g,'&lt;')}<span class="fmk-tag-close" data-nick="${nick}">&times;</span>`;
    ...
    const loadUI = () => {
        ...
        const memoBlkHtml = `
            <div class="fmk-block-section" id="section_user_memo">
                <div class="fmk-opt-row">
                    <div class="fmk-opt-label">유저 메모</div>
                    <label class="fmk-switch"><input type="checkbox" id="fmk_opt_use_memo"><span class="fmk-slider"></span></label>
                </div>
                <div class="fmk-input-group" style="gap:5px;">
                    <input type="text" class="fmk-input-text memo-nick" placeholder="닉네임" style="flex:4;">
                    <input type="text" class="fmk-input-text memo-val" placeholder="메모" style="flex:6;">
                    <button class="fmk-btn memo-add-btn">등록</button>
                </div>
                <div class="fmk-tag-container"></div>
            </div>`;

        panel.innerHTML = `<div class="fmk-panel-header"><span>Settings</span><span class="fmk-panel-close">&times;</span></div><div class="fmk-panel-body"><div class="fmk-opt-info" style="text-align:center;margin-bottom:15px">대부분의 설정은 새로고침 후에 적용됩니다. 필요없는 옵션은 끄세요.</div><div class="fmk-opt-row"><div class="fmk-opt-label">파워링크 제거</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_powerlink"><span class="fmk-slider"></span></label></div><div class="fmk-opt-info">Unicorn Pro나 갤필터 사용시 꼭 끄세요.</div><div class="fmk-opt-row"><div class="fmk-opt-label">핫딜 제휴링크 변환 방지</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_affiliate"><span class="fmk-slider"></span></label></div><div class="fmk-opt-info">AdGuard 추적보호필터, 갤필터 사용시 필요하지 않습니다.</div><div class="fmk-opt-row"><div class="fmk-opt-label">캡쳐방지 및 워터마크 해제</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_watermark"><span class="fmk-slider"></span></label></div><div class="fmk-opt-info">AdGuard 기타방해요소필터 사용시 필요하지 않습니다.</div><div class="fmk-opt-row"><div class="fmk-opt-label">광고 건너뛰고 잉포 수령</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_skipad"><span class="fmk-slider"></span></label></div><div class="fmk-opt-info">광고차단기가 켜져 있어도 작동합니다.</div><div class="fmk-opt-row"><div class="fmk-opt-label">댓글 이미지 임베딩</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_imgembed"><span class="fmk-slider"></span></label></div><hr style="border:0;border-top:1px solid ${isDark?'#444':'#eee'};margin:15px 0;">
        ${blkHtml('section_block_user', '유저 차단')}
        ${memoBlkHtml}
        ${blkHtml('section_block_keyword', '게시물/댓글 키워드 차단')}
        <hr style="border:0;border-top:1px solid ${isDark?'#444':'#eee'};margin:15px 0;"><div class="fmk-opt-row"><div class="fmk-opt-label">공지사항 차단</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_block_notice"><span class="fmk-slider"></span></label></div><div class="fmk-opt-row"><div class="fmk-opt-label">검색 어시스턴트 차단</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_block_assist"><span class="fmk-slider"></span></label></div><div class="fmk-opt-row"><div class="fmk-opt-label">포텐 메인 Navi 차단</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_block_nav"><span class="fmk-slider"></span></label></div><div class="fmk-opt-row"><div class="fmk-opt-label">최근방문 게시판 목록 차단</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_block_recent"><span class="fmk-slider"></span></label></div><div class="fmk-opt-row"><div class="fmk-opt-label">새 포텐 알림 차단</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_block_alert"><span class="fmk-slider"></span></label></div><hr style="border:0;border-top:1px solid ${isDark?'#444':'#eee'};margin:15px 0;"><div class="fmk-opt-row"><div class="fmk-opt-label">댓글 자동삭제</div><label class="fmk-switch"><input type="checkbox" id="fmk_opt_autodel_comment"><span class="fmk-slider"></span></label></div><div class="fmk-opt-info">댓글 작성목록에서 사용. 캡챠 자동해제는 지원하지 않습니다.</div><div class="fmk-opt-row"><div class="fmk-opt-label">전체 설정 백업</div><div style="display:flex;gap:5px;"><div class="fmk-icon-btn" id="fmk-export-all" title="내보내기"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg></div><div class="fmk-icon-btn" id="fmk-import-all" title="불러오기"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg></div><input type="file" accept=".json,.txt" style="display:none" id="fmk-import-file-all"></div></div></div>`;
    ...
    const bindUI = () => {
        const bind = (id, key, sec) => {
            ...
            el.addEventListener('change', (e) => {
                ...
                if (key in flags) flags[key] = e.target.checked;
                if (sec) {
                    const s = qs('#' + sec);
        ...
        _refreshTags = () => {
            ['section_block_user', 'section_block_keyword', 'section_user_memo'].forEach(sid => {
                ...
                let k;
                if (sid === 'section_block_user') k = 'blockUserList';
                else if (sid === 'section_block_keyword') k = 'blockKeywordList';
                else k = 'userMemoDict';
                renderTags(s, k);
            });
        };
        ...
        if (memoSec) {
            const nickInp = qs('.memo-nick', memoSec);
            const valInp = qs('.memo-val', memoSec);
            const mAddBtn = qs('.memo-add-btn', memoSec);
            mAddBtn.addEventListener('click', () => {
                const n = nickInp.value.trim();
                const v = valInp.value.trim();
                if (!n || !v) return;
                const dict = getDict('userMemoDict');
                dict[n] = v;
        ...
        const exportAllBtn = qs('#fmk-export-all');
        const importAllBtn = qs('#fmk-import-all');
        const importFileAll = qs('#fmk-import-file-all');

        if (exportAllBtn) {
            exportAllBtn.addEventListener('click', () => {
                const data = {};
                Object.keys(defs).forEach(k => { data[k] = getVal(k); });
            ...
            importFileAll.addEventListener('change', (e) => {
                ...
                reader.onload = (ev) => {
                    try {
                        ...
                        const checkMap = [
                            ['fmk_opt_powerlink', 'removePowerLink'], ['fmk_opt_affiliate', 'preventAffiliate'],
                            ['fmk_opt_watermark', 'noWatermark'], ['fmk_opt_skipad', 'skipAd'], ['fmk_opt_imgembed', 'imgEmbed'],
                            ['fmk_opt_block_notice', 'blockNotice'],
                            ['fmk_opt_block_assist', 'blockSearchAssist'], ['fmk_opt_block_nav', 'blockNav'],
                            ['fmk_opt_block_recent', 'blockRecent'], ['fmk_opt_block_alert', 'blockFmAlert'],
                            ['fmk_opt_block_user', 'blockUser'], ['fmk_opt_block_keyword', 'blockKeyword'],
                            ['fmk_opt_use_memo', 'useMemo'], ['fmk_opt_autodel_comment', 'autoDelComment']
                        ];
                        checkMap.forEach(([id, key]) => {
                            const el = qs('#' + id);
                            if (el) el.checked = getVal(key);
                        });
                        updLists();
                        if (_refreshTags) _refreshTags();

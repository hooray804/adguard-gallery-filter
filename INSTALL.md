...
2. 첫번째 `광고와 팝업 차단` 메뉴 선택
3. 우측 상단 `⋮` 버튼 클릭
4. `필터 편집기 열기` 버튼 클릭
5. 목록 왼쪽 하단의 `+` 버튼 클릭
6. `URL로 필터 추가` 버튼 클릭
7. 텍스트 필드에 `https://raw.githubusercontent.com/hooray804/adguard-gallery-filter/refs/heads/main/filter.txt` 입력
8. `신뢰할 수 있는 필터임` 체크 후 하단 `추가` 버튼 클릭
...
#### 스크립트 관리자(유저스크립트 매니저, Userscript Manager)가 필요한 이유

브라우저는 보안상 외부 스크립트가 웹사이트의 내부 데이터에 무분별하게 접근하는 것을 차단합니다. 따라서 유저스크립트를 안전하게 실행하고 관리하기 위해 '매니저(Tampermonkey, Violentmonkey, wBlock 등)'라는 중간 매개체가 필요합니다.
